"""
AirCanvas CLI: argument parsing and the top-level entry point.

Lives here (rather than only in the repo-root main.py) so it's a
proper importable module -- used by main.py, by `python -m aircanvas`
(see aircanvas/__main__.py), and by the `aircanvas` console script
installed via pyproject.toml, without duplicating logic three times.
...
H toggles the in-app help overlay. See README.md for the full control list.
"""
